...
const Invoices = ({ user }) => {
  ...
  const [items, setItems] = useState([
    { description: "", amount: "", isOptional: false },
  ]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!invRes.ok)
        throw new Error(invData.message || "Failed to fetch invoices");
      ...
      if (user.role === "admin") {
        // Fetch service records that have been assessed
        const recRes = await fetch(`${API}/services`, { headers });
        if (recRes.ok) {
          const recData = await recRes.json();
          // Filter: only Assessed records without an existing invoice
          const existingAptIds = (enhancedData || [])
            .map((i) => i.appointmentId?._id || i.appointmentId)
            .filter(Boolean)
            .map(String);
          const available = (recData || []).filter(
            (r) =>
              r.assessmentStatus === "Assessed" &&
              r.appointmentId &&
              !existingAptIds.includes(
                r.appointmentId._id?.toString() || r.appointmentId.toString(),
              ),
          );
          setServiceRecords(available);
        }
  ...
  // Item management
  const addItem = () =>
    setItems([...items, { description: "", amount: "", isOptional: false }]);
  const removeItem = (idx) => setItems(items.filter((_, i) => i !== idx));
  const updateItem = (idx, field, value) => {
    const updated = [...items];
    updated[idx] = {
      ...updated[idx],
      [field]: field === "isOptional" ? value : value,
    };
    setItems(updated);
  };
  ...
  const handleAddSubmit = async (e) => {
    e.preventDefault();
    if (!selectedAptId) {
      setError("Please select an assessed service record.");
      return;
    }
    ...
    try {
      const record = serviceRecords.find(
        (r) =>
          r.appointmentId._id === selectedAptId ||
          r.appointmentId === selectedAptId,
      );
  ...
  const handleReject = async (id) => {
    if (
      !window.confirm(
        "Rejecting this invoice will cancel the appointment. Continue?",
      )
    )
      return;
    try {
  ...
  const updatePaymentStatus = async (id, paymentStatus) => {
    try {
      ...
      if (res.ok) {
        setInvoices((prev) =>
          prev.map((inv) => (inv._id === id ? { ...inv, paymentStatus } : inv)),
        );
      } else {
        const d = await res.json();
  ...
  const totalOfItems = (itemsArr) =>
    (itemsArr || []).reduce((s, i) => s + (i.amount || 0), 0);
  ...
  const filtered = invoices.filter((inv) => {
    ...
    return (
      (vn.includes(search) || m.includes(search)) &&
      (filterStatus === "All" || inv.paymentStatus === filterStatus)
    );
  });

  const selectedRecord = serviceRecords.find(
    (r) =>
      r.appointmentId._id === selectedAptId ||
      r.appointmentId === selectedAptId,
  );
  ...
  return (
    <div style={{ animation: "fade-in 0.45s ease-out both" }}>
      ...
      >
        <h2 className="oswald" style={{ fontSize: "2rem", color: "#fff" }}>
          INVOICES
        </h2>
        {user.role === "admin" && (
          <button
      ...
      {showAddForm && user.role === "admin" && (
        ...
        >
          {/* Appointment/Record selector */}
          <div>
            <label style={lbl}>Select Assessed Service Record</label>
            <select
              value={selectedAptId}
              onChange={(e) => {
                ...
                if (aptId) {
                  const rec = serviceRecords.find(
                    (r) =>
                      r.appointmentId._id === aptId ||
                      r.appointmentId === aptId,
                  );
                  if (rec) {
                    const newItems = [];
                    (rec.necessaryItems || []).forEach((i) =>
                      newItems.push({
                        description: i.description,
                        amount: i.amount,
                        isOptional: false,
            ...
            >
              ...
              {serviceRecords.map((r) => (
                <option
                  key={r._id}
                  value={r.appointmentId._id || r.appointmentId}
                >
                  {r.vehicleId?.model} ({r.vehicleId?.vehicleNumber}) —{" "}
                  {r.appointmentId?.serviceType}
                </option>
              ))}
            </select>
          </div>
          ...
          {selectedRecord && (
            ...
            >
              <strong style={{ color: "#fff" }}>
                {selectedRecord.vehicleId?.model}
              </strong>{" "}
              — {selectedRecord.vehicleId?.vehicleNumber}
              <span style={{ margin: "0 10px", color: "#444" }}>|</span>
              Owner:{" "}
              <strong style={{ color: "#fff" }}>
                {selectedRecord.vehicleId?.userId?.name || "N/A"}
              </strong>
              <div
                style={{
                  marginTop: "8px",
              ...
              >
                <strong style={{ color: "#ccc" }}>Problems Assessed: </strong>{" "}
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    ₹
                    {(inv.approvalStatus === "Pending Approval"
                      ? inv.items
                          .filter((i) => i.selected !== false)
                          .reduce((s, i) => s + Number(i.amount || 0), 0)
                      : inv.totalAmount
                  ...
                  <p style={{ color: "#aaa", fontSize: "0.85rem", margin: 0 }}>
                    ...
                    {user.role !== "customer" &&
                      inv.vehicleId?.userId?.name && (
                        <span style={{ color: "#666" }}>
                          {" "}
                          | Owner: {inv.vehicleId.userId.name}
                        </span>
                      )}
                  </p>
                </div>
                      ...
                      >
                        {user.role === "customer" &&
                          ...
                          item.isOptional && (
                            <input
                              ...
                              onChange={(e) =>
                                toggleItem(inv._id, idx, e.target.checked)
                              }
                              style={{
                                accentColor: "#f39c12",
                                cursor: "pointer",
                        ...
                        <span
                          style={{
                            color: "#ccc",
                            textDecoration:
                              item.selected === false ? "line-through" : "none",
                          }}
                        >
                          {item.description}
                        </span>
